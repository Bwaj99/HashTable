...
int HashTable::computeHash(const string &s) {
    ...
    for (unsigned int i = 0; i < s.size(); ++i) {
        //hash = (hash * pow(32, i)) + s.at(i);
        hash = hash + (static_cast<int>(s[i]) * pow(32,i));
    }
...
void HashTable::put(const string &s, int score) {
    ...
    list<WordEntry>::iterator i = hashTable[stringHash].begin();
    if (contains(s)){ 
        while (i != hashTable[stringHash].end()){
            if (i->getWord() == s){
        ...
        hashTable[stringHash].push_back(*newWord);
    }
}
